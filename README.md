...
4. "Rẽ sai hướng" được định nghĩa theo logic hình học thông qua các _turn/exit regions_ (polygon hoặc vùng quyết định) cấu hình thủ công trong mỗi camera/lane. Nếu maneuver thực tế không thuộc `allowed_maneuvers` thì violation loại `turn_left_not_allowed`, `turn_right_not_allowed`, `turn_...`.
