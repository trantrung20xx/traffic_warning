				...
				{options.map((option) => {
					...
					return (
						<label
							...
							className={
								disabled
									? "lane-change-option lane-change-option-disabled"
									: "lane-change-option"
							}>
							<input
								type="checkbox"
							...
							/>
							<span className="lane-change-option-label">
								{getLabel(option)}
							</span>
							{note ? (
								<span className="lane-change-option-note">{note}</span>
							) : null}
						</label>
					);
				})}
...
function getToastTone(message) {
	...
	if (
		...
		normalized.includes("đang được hệ thống sử dụng")
	) {
		return "success";
	}
	return "info";
...
const VALIDATION_MESSAGE_BY_CODE = {
	...
	LANE_OVERLAP_DANGEROUS:
		"Các làn đang chồng lên nhau nhiều, hệ thống có thể nhận nhầm xe thuộc làn nào.",
...
const VALIDATION_SUGGESTION_BY_CODE = {
	LANE_POLYGON_INVALID:
		"Vẽ lại biên làn bằng ít nhất 3 điểm và tạo thành vùng rõ ràng.",
...
function cleanValidationText(text) {
	return String(text || "")
		...
		.replaceAll("direction path", "đường hướng đúng chiều")
		.replaceAll("direction check zone", "vùng kiểm tra hướng")
...
function normalizeValidationIssue(issue, index) {
	...
	return {
		...
		message:
			VALIDATION_MESSAGE_BY_CODE[code] ||
			cleanValidationText(source.message || "Cấu hình cần được kiểm tra lại."),
		suggestion:
			VALIDATION_SUGGESTION_BY_CODE[code] ||
			cleanValidationText(source.suggestion || ""),
	};
}
...
function removeRedundantValidationIssues(issues, lanesById) {
	...
	return issues.filter((issue) => {
		if (issue.code === "MISSING_EXIT_CONFIRM") {
			...
			const maneuverCfg =
				lane && issue.maneuverKey
					? lane.maneuvers?.[issue.maneuverKey] || null
					: null;
			if (maneuverCfg) {
				const hasExitLine =
					Array.isArray(maneuverCfg.exit_line) &&
					maneuverCfg.exit_line.length >= 2;
				const hasExitZone =
					Array.isArray(maneuverCfg.exit_zone) &&
					maneuverCfg.exit_zone.length >= 3;
...
function groupValidationIssues(issues) {
	...
	return Array.from(groups.values())
		...
		.sort(
			(a, b) =>
				validationGroupSortValue(a.label) - validationGroupSortValue(b.label) ||
...
function ValidationIssuesPanel({ issues, lanes }) {
	...
	const groups = useMemo(
		() => groupValidationIssues(normalizedIssues),
		[normalizedIssues],
	);
		...
		<section className={`validation-issues-panel ${dominantLevel}`}>
			<button
				...
				<span className="validation-summary-text">
					{getValidationSummaryText(counts)}
				</span>
				...
				<div className="validation-detail-panel">
					{groups.map((group) => (
						<div key={group.label} className="validation-detail-group">
							<div className="validation-detail-group-title">
								{group.label}
							</div>
							<div className="validation-detail-list">
								{group.items.map((issue) => (
									<div
										key={issue.id}
										className={`validation-detail-item ${issue.level}`}>
										<span
											className="validation-detail-level"
										...
										<div className="validation-detail-copy">
											...
											{issue.suggestion ? (
												<div className="validation-detail-suggestion">
													Cách xử lý: {issue.suggestion}
												</div>
											) : null}
										</div>
									</div>
...
export default function ManagementView({
	...
	useEffect(() => {
		...
		fetchCameraDetail(targetId)
			.then((detail) => {
				...
				resetDraftHistory();
				setMessage(
					detail.runtime_applied
						? "Cấu hình hiện tại đang được hệ thống sử dụng."
						: "",
				);
			})
			.catch(() => {
	...
	const selectedLane = useMemo(
		() =>
			draft.lane_config.lanes.find((lane) => lane.lane_id === selectedLaneId) ||
			draft.lane_config.lanes[0] ||
			null,
	...
	const selectedDirectionRule = useMemo(
		() => normalizeDirectionRuleConfig(selectedLane?.direction_rule),
		[selectedLane],
	);
	...
	const polygonStatus = useMemo(() => {
		...
		const targetLabel = getEditTargetLabel(
			editTarget,
			selectedLane?.lane_id ?? null,
			selectedManeuver,
		);
		const warnings = [];
		...
		if (selectedPoints.length > 0 && selectedPoints.length < minimumPoints) {
			warnings.push(
				`${targetLabel} hiện có dưới ${minimumPoints} điểm, chưa đủ để tạo ${geometryNoun} hợp lệ.`,
			);
		}
		if (
			isPolygonEditTarget(editTarget) &&
			selectedPoints.length >= 4 &&
			polygonSelfIntersects(selectedPoints)
		) {
			warnings.push(
				`${targetLabel} đang tự cắt nhau, nên chỉnh lại để tránh vùng hình học khó kiểm soát.`,
			);
		}
		return { warnings };
	...
	const updateDraft = (updater, options = {}) => {
		setDraft((current) => {
			...
			const sanitizedLanes = next.lane_config.lanes.map((lane) => ({
				...
				allowed_lane_changes: (
					lane.allowed_lane_changes || [lane.lane_id]
				).filter((value) => laneIds.includes(Number(value))),
			}));
			return {
				...next,
				camera: {
				...
				lane_config: {
					...
					frame_width:
						Number(next.camera.frame_width) || next.lane_config.frame_width,
					frame_height:
						Number(next.camera.frame_height) || next.lane_config.frame_height,
		...
		if (options.markDirty !== false) {
			...
			if (options.showDirtyMessage !== false) {
				setMessage(
					'Đang có thay đổi chưa lưu. Nhấn "Lưu cấu hình làn đường" để áp dụng cấu hình mới.',
				);
			}
		}
	};
	...
	const updateLane = (laneId, updater, options = {}) => {
		updateDraft(
			(current) => ({
				...
				lane_config: {
					...current.lane_config,
					lanes: current.lane_config.lanes.map((lane) =>
						lane.lane_id === laneId ? updater(lane) : lane,
					),
				},
			}),
			options,
	...
	const updateSelectedManeuverConfig = (updater) => {
		...
		updateLane(selectedLane.lane_id, (lane) => {
			...
			const nextCfg = {
				...updatedCfg,
				allowed:
					Boolean(updatedCfg.enabled ?? true) &&
					Boolean(updatedCfg.allowed ?? false),
			};
	...
	const updateSelectedDirectionRule = (updater) => {
		if (!selectedLane) return;
		updateLane(selectedLane.lane_id, (lane) => {
			const currentRule = normalizeDirectionRuleConfig(lane.direction_rule);
			const nextRule = normalizeDirectionRuleConfig(updater(currentRule));
			return {
				...lane,
				direction_rule: nextRule,
			};
		});
	};
	...
	const addLane = () => {
		...
		updateDraft((current) => ({
			...
			lane_config: {
				...current.lane_config,
				lanes: [
					...current.lane_config.lanes,
					createEmptyLane(
						nextLaneId,
						current.camera.frame_width,
						current.camera.frame_height,
					),
				],
			},
		}));
	...
	const removeLane = (laneId) => {
		const remaining = draft.lane_config.lanes.filter(
		...
		updateDraft((current) => ({
			...
			lane_config: {
				...current.lane_config,
				lanes: current.lane_config.lanes.filter(
					(lane) => lane.lane_id !== laneId,
				),
			},
		}));
	...
	const updateTargetGeometry = (updater) => {
		...
		if (editTarget === "direction_path" || editTarget === "direction_check_zone") {
			updateLane(
				selectedLane.lane_id,
				(lane) => {
					const currentRule = normalizeDirectionRuleConfig(lane.direction_rule);
		...
		updateLane(
			...
			(lane) => {
				if (
					...
					const currentManeuvers = lane.maneuvers || {};
					const currentManeuverCfg = currentManeuvers[
				...
				if (
					entry.editTarget === "direction_path" ||
					entry.editTarget === "direction_check_zone"
				) {
					const currentRule = normalizeDirectionRuleConfig(lane.direction_rule);
					return {
						...lane,
						direction_rule: {
							...
							[entry.editTarget === "direction_path"
								? "direction_path"
								: "check_zone"]: cloneDraftState(points),
						},
					};
				}
				...
				return {
					...lane,
					[entry.editTarget === "lane_polygon" ? "polygon" : entry.editTarget]:
						cloneDraftState(points),
				};
			},
			{ showDirtyMessage: false },
		);
	...
	const undoDraftChange = () => {
		...
		redoStackRef.current = [
			...redoStackRef.current.slice(-(MAX_DRAFT_HISTORY - 1)),
			entry,
		];
		setHistoryRevision((value) => value + 1);
		applyGeometryHistoryEntry(
			entry,
			entry.beforePoints,
			"Đã quay lại thao tác polygon trước đó.",
		);
	};
	...
	const redoDraftChange = () => {
		...
		applyGeometryHistoryEntry(
			entry,
			entry.afterPoints,
			"Đã làm lại thao tác polygon vừa hoàn tác.",
		);
	};
	...
	const startCreateCamera = () => {
		requestDiscardChanges(
			"Tạo camera mới sẽ bỏ các chỉnh sửa hiện chưa lưu trên camera đang mở.",
			() => {
				setIsNewCamera(true);
				setActiveCameraId(null);
				const draftState = createCameraDraft(
					`cam_${String(cameras.length + 1).padStart(2, "0")}`,
				);
				setDraft(draftState);
				setSelectedLaneId(1);
				setSelectedManeuver("straight");
	...
	const handleDelete = () => {
		...
		setConfirmDialog({
			...
			description:
				"Camera, cấu hình làn, ảnh nền và ảnh bằng chứng liên quan sẽ bị xóa khỏi hệ thống.",
	...
	const handleSelectCamera = (cameraId) => {
		requestDiscardChanges(
			"Chuyển sang camera khác sẽ bỏ các chỉnh sửa hiện chưa lưu trên camera đang mở.",
			() => {
				setIsNewCamera(false);
			...
			<div className="management-layout">
				<aside className="panel sidebar-panel management-sidebar">
					<div className="panel-header compact management-sidebar-header">
						...
						<button
							className="button secondary management-add-camera-button"
							onClick={startCreateCamera}>
							<AppIcon name="plus" />
							Thêm camera
						</button>
					</div>
						...
						<div className="panel-header">
							<div>
								...
								<div className="title-with-icon">
									<span className="panel-title-icon">
										<AppIcon
											name={isNewCamera ? "plus" : "settings"}
											size={20}
										/>
									</span>
									<h2>
										{isNewCamera
											? "Thêm camera mới"
											: `Chỉnh sửa ${draft.camera.camera_id || "camera"}`}
									</h2>
								</div>
							</div>
							...
							<section className="management-subcard">
								...
								<div className="form-grid management-form-grid">
									...
									<label className="field">
										...
										<input
											value={
												draft.camera.location.intersection_name
											}
											onChange={(event) =>
												updateDraft((current) => ({
													...current,
					...
					<section className="panel lane-panel">
						<div className="panel-header">
							<div>
								...
								<div className="title-with-icon">
									...
									<h3>
										Cấu hình làn đường và các vùng phục vụ nhận diện
										hướng rẽ
									</h3>
								</div>
							</div>
						...
						<div className="lane-editor-grid">
							<div className="lane-list-panel">
										...
										<div
											...
											className={
												lane.lane_id === selectedLaneId
													? "lane-chip lane-chip-row active"
													: "lane-chip lane-chip-row"
											}>
											<button
												className="lane-chip-main"
												onClick={() =>
								...
								<button
									className="button secondary lane-add-button"
									onClick={addLane}>
							...
							{selectedLane ? (
									...
									<section className="management-subcard">
										...
										<div className="inline-fields lane-inline-fields">
											<label className="field lane-field-changes">
												...
												<MultiSelectDropdown
													summary={draft.lane_config.lanes
														.filter((laneOption) =>
															(
																selectedLane.allowed_lane_changes ||
																[]
															).includes(
																laneOption.lane_id,
															),
													...
													isChecked={(laneOption) =>
														(
															selectedLane.allowed_lane_changes ||
															[]
														).includes(laneOption.lane_id)
													}
													isDisabled={(laneOption) =>
														laneOption.lane_id ===
														selectedLane.lane_id
													}
													getNote={(laneOption) =>
											...
											<label className="field lane-field-vehicles">
												...
												<MultiSelectDropdown
													summary={VEHICLE_TYPES.filter(
														(vehicleType) =>
															(
																selectedLane.allowed_vehicle_types ||
																[]
									...
									<section className="management-subcard">
										...
										<div className="checkbox-group">
											<label className="checkbox-pill">
												...
												<span>
													Bật theo dõi{" "}
													{getManeuverLabel(selectedManeuver)}
												</span>
											</label>
											<label
												className={
											...
											<label className="checkbox-pill">
												<input
													type="checkbox"
													checked={selectedDirectionEnabled}
													onChange={(event) =>
														updateSelectedDirectionRule(
															(rule) => ({
																...rule,
																enabled:
																	event.target.checked,
												...
												/>
												<span>Kiểm tra ngược chiều</span>
											</label>
										</div>
									</section>
										...
										<div className="editor-toolbar-row">
											<div className="editor-action-toolbar compact-toolbar">
												...
												<div className="editor-actions tight-actions">
													<label
														className={`button secondary compact-button smaller-button${isNewCamera || backgroundBusy ? " disabled" : ""}`}>
														<input
															type="file"
													...
													<div
														className={
															hasBackgroundImage
																? "badge success toolbar-badge"
																: "badge subtle toolbar-badge"
														}>
														<AppIcon
															name={
																hasBackgroundImage
											...
											<div className="editor-action-toolbar compact-toolbar">
												...
												<div className="editor-actions tight-actions">
													<button
														...
														onClick={() => {
															setPolygonLocked((value) => {
																...
																setMessage(
																	nextValue
																		? "Đã khóa polygon để tránh chỉnh nhầm."
																		: "Đã mở khóa polygon để tiếp tục chỉnh.",
																);
																return nextValue;
															});
														}}>
													...
													<div
														className="polygon-history-buttons"
														role="group"
														aria-label="Hoàn tác và làm lại thao tác">
														<button
								...
								<div className="empty-state slim">
									Tạo làn đầu tiên để bắt đầu cấu hình.
								</div>
							)}
						</div>
