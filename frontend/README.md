...
## Cấu hình mà người dùng thao tác trên UI (lane-centric)
...
| `approach_zone` | Vùng tiếp cận dùng khóa lane nguồn trước khi commit turn. |
| `commit_gate` | Vùng bắt đầu maneuver (optional). |
| `commit_line` | Vạch bắt đầu maneuver (optional). |
| `allowed_lane_changes` | Làn được phép chuyển tới (rule `wrong_lane`). |
| `allowed_vehicle_types` | Loại xe hợp lệ trong lane. |
| `allowed_maneuvers` | Hướng đi hợp lệ theo luật cho lane. |

### Cấp maneuver (`straight`, `left`, `right`, `u_turn`)
...
| `exit_line` | Vạch xác nhận xe ra nhánh maneuver. |
| `exit_zone` | Vùng xác nhận xe ra nhánh maneuver. |
