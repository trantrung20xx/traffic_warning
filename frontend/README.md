...
- Quản lý camera và cấu hình làn/hướng di chuyển.
...
## Cấu hình người dùng thao tác trên UI (theo làn đường)
...
| `allowed_maneuvers` | Hướng đi hợp lệ theo luật cho lane. |

### Nhóm cấu hình theo hướng di chuyển (`straight`, `left`, `right`, `u_turn`)

| Trường | Giải thích |
|---|---|
| `enabled` | Bật/tắt nhận diện hướng đi. |
| `allowed` | Cho phép/cấm hướng đi theo luật. |
| `movement_path` | Đường đi kỳ vọng (polyline) để backend suy corridor/evidence. |
...
| `exit_line` | Vạch xác nhận xe đã đi ra đúng nhánh của hướng đi. |
| `exit_zone` | Vùng xác nhận xe đã đi ra đúng nhánh của hướng đi. |
...
| `GET` | `/api/cameras/{camera_id}` | Lấy chi tiết camera + lane config + validation. |
...
| `GET` | `/api/cameras/{camera_id}/trajectories` | Lấy trajectory overlay hỗ trợ cấu hình. |
...
| `src/views/ManagementView.jsx` | Màn hình cấu hình camera/làn/hướng di chuyển. |
