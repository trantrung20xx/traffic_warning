...
| Đọc biển số | Tìm vùng biển số và đọc chữ/số nếu bật cấu hình OCR. |
| Lưu dữ liệu | Ghi vi phạm vào SQLite và lưu ảnh bằng chứng. |
| Gửi realtime | Gửi xe đang theo dõi và vi phạm mới qua WebSocket. |
...
9. CameraContext lưu DB, lưu ảnh, gửi WebSocket
```

Nói đơn giản:

```text
Frame -> Xe -> Làn -> Luật -> Vi phạm -> Ảnh bằng chứng -> UI
```
...
| Trường | Ý nghĩa | Gợi ý |
|---|---|---|
| `weights_path` | File model YOLO nhận diện phương tiện. | Ví dụ `backend/yolov8m.pt`. |
...
| `GET` | `/api/cameras/{camera_id}/lanes` | Lấy lane dạng pixel để vẽ overlay. |
| `GET` | `/api/cameras/{camera_id}/trajectories` | Lấy quỹ đạo gần đây. |
| `GET` | `/api/cameras/{camera_id}/preview` | Xem camera dạng MJPEG. |
...
| `WS /ws/violations?camera_id=...` | Vi phạm mới realtime. |
