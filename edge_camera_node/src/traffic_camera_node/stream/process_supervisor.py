...
class ProcessSupervisor:
    def __init__(
        ...
        # Cờ đánh thức vòng giám sát ngay khi có lệnh điều khiển từ API/UI.
        ...
        # Theo dõi tình trạng tiến trình còn sống nhưng không phát được frame thật.
    ...
    def _run(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if not self._stream_enabled:
                ...
                if fps <= 0.1:
                    self._state.set_stream_running(False)
                    self._state.set_warning("No live RTSP frames detected from pipeline.")
                    now = time.monotonic()
                    if self._no_frame_warning_since <= 0.0:
                        self._no_frame_warning_since = now
                    elif now - self._no_frame_warning_since >= 6.0:
                        self._no_frame_warning_since = 0.0
                        self._try_restart_with_limits("no live RTSP frames detected", ignore_retry_delay=True)
                elif fps < self._config.watchdog.fps_warning_threshold:
                    self._state.set_stream_running(True)
                    self._no_frame_warning_since = 0.0
                    ...
                    self._state.set_stream_running(True)
                    self._no_frame_warning_since = 0.0
                    if self._state.get_status() == NodeStatus.WARNING:
                        self._state.transition(NodeStatus.STREAMING)
